  ...
  "sessions": [
    {
      ...
      "evidence_picture_description": "Approximately 100-word description of the evidence picture to generate"
    }
  ]
}
```
...
- In the Support Notes section, after `support_notes` the generator adds a
  ...
  info", followed by `support_note_summary` and then
  `evidence_picture_description`. No image is embedded or generated by this
  tool — `evidence_picture_description` is only text, intended for a
  separate image-generation step. Section 4 also uses `support_note_summary`
  (see below) but ignores `evidence_picture_description`.
